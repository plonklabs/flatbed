...
pub(crate) fn generate(ops: &[FbOperation]) -> String {
    ...
    // The codec import is only referenced by operations that carry a typed
    // request or response; emitting it otherwise trips `noUnusedLocals`.
    if ops
        .iter()
        .any(|op| op.request_type.is_some() || op.response_type.is_some())
    {
