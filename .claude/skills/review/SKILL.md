...
3. **Walk the [Quality checklist](#quality-checklist)** and fix the comment AND any adjacent issues that fall under the same category. A "constant duplication" comment on one literal should trigger a sweep for other duplications in the same file; a "hardcoded value" comment should trigger a sweep for siblings.
...
10. **Reactive vs comprehensive.** When fixing any one comment, look at the whole file. The issue the reviewer flagged usually has siblings. The `feedback_class_level_fixes.md` memory captures this for regression fixes: "On regression fixes, grep siblings with the same wire shape." Apply the same lens to review comments.
