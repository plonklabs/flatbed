...
- **`pure-cleanup: yes/no`** — yes if the entire PR is formatting / dead-code deletion / comment hygiene / dependency bumps with no behaviour change. Pure-cleanup PRs skip both `tests` and `smoke`.
...
- **`tests: required/skip`** — the automated analog of smoke:
  - `required` (run `cargo test --workspace`, plus `--all-features` when feature-gated code is touched) for any PR with code changes.
  - additionally run `bash scripts/check-generated.sh` when the PR touches `.fbs` schemas or the `flatbed_build` codegen — it verifies the committed generated code still matches the schemas.
  - `skip` for docs-only / pure-cleanup PRs.

  `tests` and `smoke` are independent: a PR can need both, either, or neither.
...
3. Run the gate suite locally (`flatc` on `PATH`):
   ```bash
   cargo fmt --all
...
Per [[feedback_draft_pr_is_checkpoint]] — push as soon as the code compiles and unit tests pass. Smoke doesn't gate the draft push (it runs in Lane B later).
...
3. Re-run `cargo fmt`, `cargo clippy -- -D warnings`, and the relevant `cargo test` / per-package build after the fix commits.
...
Bot review (`/review --auto`) and the smoke pipeline (tests + smoke) run **concurrently** against the same HEAD to minimize wall-clock. Doing them serially wastes time when both take minutes.
...
**Lane B — artifact smoke pipeline** (background, started right after 2c flips ready). Sequential internally (smoke builds on green tests), but the whole pipeline runs **in parallel with Lane A**:

- **B1: tests** (when `tests: required`). Run `cargo test --workspace` (add `--all-features` when feature-gated code changed) via Bash `run_in_background`; watch with `Monitor` / `TaskOutput`. Run `bash scripts/check-generated.sh` when schemas/codegen changed. On green, proceed to B2; on red, see Failure handling.
- **B2: smoke** (when `smoke: required`) — **build and exercise the real artifact end-to-end**, matched to what the PR changed. There is no cluster; the artifact is a process you run on this host.
...
- **Skipping**: if both `tests: skip` AND `smoke: skip` (typical for pure cleanup or docs-only), Lane B is a no-op for this PR.
...
- Lane B green (where required): tests green, smoke green.
- Local gates green: `cargo test` + clippy `-D warnings` + fmt clean (+ `check-generated.sh` when relevant).
...
- For each PR, state which gates ran: bot review (always), tests (yes/no + result), smoke (yes/no + what was exercised and the observed signal).
...
- Lane B (tests + smoke) starts right after flipping ready and runs concurrently with Lane A's bot-review loop. They are sequenced internally (smoke builds on green tests).
...
- Merge gates: `/review --auto` green AND tests green (when required) AND smoke green (when required) AND fmt + clippy `-D warnings` + `cargo test` green at HEAD. All must hold — no exceptions.
