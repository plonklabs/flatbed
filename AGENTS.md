...
This file is the canonical shared policy for every coding agent working
in this repository; harness-specific entry points (CLAUDE.md and
equivalents) resolve here.
...
Concurrent issue delivery runs through an orchestrator/worker fleet — see
[`.agents/README.md`](.agents/README.md) for the architecture and
[`.agents/skills/`](.agents/skills/) for the workflow procedures:

- An **orchestrator** session (`/orchestrator`) owns issue routing, labels,
  the Project board, dispatch, and CI monitoring. It never edits code.
- **Workers** (`worker-flatbed1..3`) each own one issue end-to-end in a
  pinned worktree under `worktrees/`; `worktrees/flatbed4` is reserved for
  the user. Seats test fully in parallel — per-worktree target dirs and a
  per-seat NATS broker (`scripts/nats-broker.sh`); there is no serialized
  test bench.
...
- Static serving sits behind readiness: while the server is not ready, a GET
  (including the SPA shell) returns `503`, same as declared routes.
  Kubernetes won't route traffic before readiness anyway. Readiness is the
  boot latch *and* every registered `ReadinessGate`, so a runtime gate that
  closes after boot — a dropped broker connection, say — takes the SPA down
  with it, not just `/readyz`.
