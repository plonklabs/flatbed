...
16. **Conditional made unconditional.** When a change drops the condition on a branch — an `if` that gated a fallback, a feature flag, an early-exit path — so only one path remains, diff the branch about to become dead against the surviving unconditional path before treating the removal as equivalent: timing, defaults, error handling, not just the happy-path return value. A conditional fast-exit dropped this way turned a 2-second shutdown into a 30-second one in a draft PR; only an adversarial review caught it because the diff was never checked against the case it stopped handling.
